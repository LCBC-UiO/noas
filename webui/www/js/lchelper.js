...
function lcSaveSelection(sel) {
  const d = Date.parse(sel.date);
  fileName = `noas_selection_${lcGetDateFnStr(d)}_${lcGetSelHash(sel)}.json`;
...
function lcGetSelHash(sel) {
  ...
  return MD5(JSON.stringify(sel)).substr(0,7);
}