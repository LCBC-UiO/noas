...
BEGIN
  IF NEW."date" = (SELECT birthdate FROM subjects WHERE id = NEW.subject_id) THEN
    RAISE EXCEPTION 'Visit date cannot be the same as subject birthdate. subject_id: %, birthdate: %, visit date: %', NEW.subject_id, (SELECT birthdate FROM subjects WHERE id = NEW.subject_id), NEW.date;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.age < 3 THEN
        RAISE EXCEPTION 'Visit age cannot be less than 3. subject_id: %, birthdate: %, visit date: %', NEW.subject_id, (SELECT birthdate FROM subjects WHERE id = NEW.subject_id), NEW.date;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
